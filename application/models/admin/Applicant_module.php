...
class Applicant_module extends Base_model
{
    public $table = "applicant_module";

    var $column_order = array(null,'id','name','email','experience','role','status','created_at','updated_at'); //set column field database for datatable orderable

    var $column_search = array('id','name','email','experience','role','status','created_at','updated_at'); //set column field database for datatable searchable 
    ...
    public function jointTables()
    {
        ...
        // $this->db->join('employee as e ', ' e.id = applicant_module.employeeId');
    ...
    public function getAction(){
        ...
        if(isset($_SESSION['role_type']) && ($_SESSION['role_type'] == 0)){
            $this->db->where('applicant_module.employeeId', $_SESSION['employeeId']);
        }
        
        if(isset($_GET['emp_id']) && !empty($_GET['emp_id'])){
            $this->db->where('applicant_module.employeeId', $_GET['emp_id']);
        }
        ...
        if(isset($_GET['type']) && $_GET['type'] == 'a'){
            $this->db->where('applicant_module.status',1);
        }
        
        if(isset($_GET['type']) && $_GET['type'] == 'c'){
            $this->db->where('applicant_module.status',2);
        }
        
        if(isset($_GET['type']) && $_GET['type'] == 's'){
            $this->db->where('applicant_module.status',3);
        }
        
        if(isset($_GET['type']) && $_GET['type'] == 'h'){
            $this->db->where('applicant_module.status',4);
        }
