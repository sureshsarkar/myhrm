...
class Leavemanagement_model extends Base_model
{
    public $table = "leave_management";

    var $column_order = array(null, 'emp_id','leave_date_from','leave_date_to','leave_reson','leave_management.status','leave_management.created_at','leave_management.updated_at','e.emp_code','e.fname','e.lname','e.phone','e.email','e.department'); //set column field database for datatable orderable

    var $column_search = array('emp_id','leave_date_from','leave_date_to','leave_reson','leave_management.status','leave_management.created_at','leave_management.updated_at','e.emp_code','e.fname','e.lname','e.phone','e.email','e.department'); //set column field database for datatable searchable 
    ...
    public function _get_datatables_query($where = NULL)
    {
        ...
        $this->db->select('leave_management.*,e.emp_code,e.fname,e.lname,e.phone,e.email');
    ...
    public function jointTables()
    {
        ...
        $this->db->join('employee as e ', ' e.id = leave_management.emp_id', 'left');
    ...
    public function getAction(){
        ...
        if(isset($_SESSION['role_type']) && ($_SESSION['role_type'] == 0)){
            $this->db->where('leave_management.emp_id', $_SESSION['employeeId']);
        }

        if(isset($_GET['type']) && $_GET['type'] == 'sick'){
            $this->db->where('leave_management.nature_of_leave','1');
        }
        
        if(isset($_GET['type']) && $_GET['type'] == 'casual'){
            $this->db->where('leave_management.nature_of_leave','2');
        }
        
        if(isset($_GET['type']) && $_GET['type'] == 'priv'){
            $this->db->where('leave_management.nature_of_leave','3');
        }

        if(isset($_GET['type']) && $_GET['type'] == 'wfh'){
            $this->db->where('leave_management.nature_of_leave','4');
        }

        if(isset($_GET['type']) && $_GET['type'] == '1'){
            $fromdate = date("Y-m-d 00:00:00");
            $curentdate=date("Y-m-d 23:59:59");
            $this->db->where('leave_management.created_at <=', $curentdate);
            $this->db->where('leave_management.created_at >=', $fromdate);
        }

        if(isset($_GET['type']) && $_GET['type'] == 'dev'){
            $this->db->where('leave_management.department','1');
        }

        if(isset($_GET['type']) && $_GET['type'] == 'design'){
            $this->db->where('leave_management.department','2');
        }

        if(isset($_GET['type']) && $_GET['type'] == 'seo'){
            $this->db->where('leave_management.department','3');
        }
        
        if(isset($_GET['type']) && $_GET['type'] == 'hr'){
            $this->db->where('leave_management.department','4');
        }

        if(isset($_GET['type']) && $_GET['type'] == 'a'){
            $this->db->where('leave_management.department','5');
        }

        if(isset($_GET['type']) && $_GET['type'] == '7'){
            $curentdate=date("Y-m-d H:i:s");
            $fromdate = strtotime("-7 day", strtotime($curentdate));
        ...
        if(isset($_GET['type']) && $_GET['type'] == '30'){
            $curentdate=date("Y-m-d H:i:s");
            $fromdate = strtotime("-30 day", strtotime($curentdate));
            $fromdate= date('Y-m-d H:i:s', $fromdate);
            $this->db->where('leave_management.created_at <=', $curentdate);
            $this->db->where('leave_management.created_at >=', $fromdate);
        }
        ...
        if(isset($_GET['type']) && $_GET['type'] == '365'){
            $curentdate=date("Y-m-d H:i:s");
            $fromdate = strtotime("-365 day", strtotime($curentdate));
            $fromdate= date('Y-m-d H:i:s', $fromdate);
            $this->db->where('leave_management.created_at <=', $curentdate);
            $this->db->where('leave_management.created_at >=', $fromdate);
        }
