...
class Employee_model extends Base_model
{
    ...
    var $column_order = array(null, 'fname','lname','email','phone','gender','dob','department','role','joining_date','ctc','created_at','updated_at'); //set column field database for datatable orderable

    var $column_search = array('fname','lname','email','phone','gender','dob','department','role','joining_date','ctc','created_at','updated_at'); //set column field database for datatable searchable 
    ...
    public function getAction(){
        ...
        if(isset($_GET['type']) && $_GET['type'] == 'new_emp'){
            $this->db->where('employee.joining_date >', $lastData);
        }
        
        if(isset($_GET['type']) && $_GET['type'] == 'v_emp'){
            $this->db->where('employee.status',1);
        }
        
        if(isset($_GET['type']) && $_GET['type'] == 'uv_emp'){
