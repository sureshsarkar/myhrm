...
{
    public $table = "emp_attandance";

    var $column_order = array('emp_attandance.attendance_date','emp_attandance.created_at','emp_attandance.updated_at','e.fname','e.lname','e.phone','e.email','e.gender','e.department','e.role','e.profile_image','e.status','e.joining_date'); //set column field database for datatable orderable

    var $column_search = array('emp_attandance.attendance_date','emp_attandance.created_at','emp_attandance.updated_at','e.fname','e.lname','e.phone','e.email','e.gender','e.department','e.role','e.profile_image','e.status','e.joining_date'); //set column field database for datatable searchable 
    ...
    public function _get_datatables_query($where = NULL)
    {
        ...
        $this->db->select('emp_attandance.*,e.fname,e.lname,e.phone,e.email,e.gender,e.department,e.role,e.profile_image,e.status,e.joining_date');
    ...
    public function jointTables()
    {
        ...
        $this->db->join('employee as e ', ' e.id = emp_attandance.employeeId');
    ...
    public function getAction(){
        ...
        if(isset($_SESSION['role_type']) && ($_SESSION['role_type'] == 0)){
            $this->db->where('emp_attandance.employeeId', $_SESSION['employeeId']);
        }
        
        if(isset($_GET['emp_id']) && !empty($_GET['emp_id'])){
            $this->db->where('emp_attandance.employeeId', $_GET['emp_id']);
        }

        if(isset($_GET['type']) && $_GET['type'] == 'this_month_a'){
            $this->db->where('salary_month', date("M-Y"));
        }
        
        if(isset($_GET['type']) && $_GET['type'] == 'last_month_a'){
