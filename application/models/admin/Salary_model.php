...
class Salary_model extends Base_model
{
    public $table = "salary";

    var $column_order = array(null, 'total_earning','month','total_deduction','net_salary','salary.status', 'salary.created_at','e.fname','e.lname'); //set column field database for datatable orderable

    var $column_search = array( 'total_earning','month','total_deduction','net_salary','salary.status', 'salary.created_at','e.fname','e.lname'); //set column field database for datatable searchable 
    ...
    public function _get_datatables_query($where = NULL)
    {
        ...
        $this->db->select('salary.id,salary.month,salary.total_earning,salary.total_deduction,salary.net_salary,salary.status, salary.created_at,e.id as e_id,e.fname,e.lname,e.profile_image');
    ...
    public function jointTables()
    {
        ...
        $this->db->join('employee as e ', ' e.id = salary.emp_id');
    ...
    public function getAction(){
        ...
        if(isset($_SESSION['role_type']) && ($_SESSION['role_type'] == 0)){
            $this->db->where('salary.emp_id', $_SESSION['employeeId']);
        }

        if(isset($_GET['type']) && $_GET['type'] == 'this_month_s'){
            $this->db->where('month', date("F"));
            $this->db->where('year ', $year);
        }
        
        if(isset($_GET['type']) && $_GET['type'] == 'last_month_s'){
            $this->db->where('month', $lastMonth);
            $this->db->where('year ', $year);
        }
        
        if(isset($_GET['type']) && $_GET['type'] == 'last_12month_s'){
