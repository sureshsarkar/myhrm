...
class Performance_model extends Base_model
{
    ...
    var $column_order = array(null, 'performance.emp_id','performance.month','performance.rating','performance.created_at','performance.updated_at','e.id','e.fname','e.lname','e.emp_code'); //set column field database for datatable orderable

    var $column_search = array('performance.emp_id','performance.month','performance.rating','performance.created_at','performance.updated_at','e.id','e.fname','e.lname','e.emp_code'); //set column field database for datatable searchable 
    ...
    public function _get_datatables_query($where = NULL)
    {
        ...
        $this->db->select('performance.*, e.id as emp_id, e.fname,e.lname,e.emp_code,e.profile_image');
    ...
    public function jointTables()
    {
        ...
        $this->db->join('employee as e ', ' e.id = performance.emp_id');
    ...
    public function getAction(){
        ...
        if(isset($_SESSION['role_type']) && ($_SESSION['role_type'] == 0)){
            $this->db->where('performance.emp_id', $_SESSION['employeeId']);
        }

        if(isset($_GET['type']) && $_GET['type'] == 'this'){
            $this->db->where('performance.month', $currentMonth);
        }
        
        if(isset($_GET['type']) && $_GET['type'] == 'last'){
            $this->db->where('performance.month', $lastMonth);
        }
        
        if(isset($_GET['type']) && $_GET['type'] == 'last12'){
            $this->db->where('month >=', date("Y-F", strtotime($last12Month)));
            $this->db->where('month <=', date("Y-F"));
        }

        if(isset($_GET['type']) && $_GET['type'] == 'dev'){
            $this->db->where('performance.department','1');
        }

        if(isset($_GET['type']) && $_GET['type'] == 'design'){
            $this->db->where('performance.department','2');
        }

        if(isset($_GET['type']) && $_GET['type'] == 'seo'){
            $this->db->where('performance.department','3');
        }
        
        if(isset($_GET['type']) && $_GET['type'] == 'hr'){
            $this->db->where('performance.department','4');
        }

        if(isset($_GET['type']) && $_GET['type'] == 'a'){
            $this->db->where('performance.department','5');
        }
